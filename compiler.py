...
class color: # for printing text in color
	PURPLE = '\033[95m'
	CYAN = '\033[96m'
	DARKCYAN = '\033[36m'
	BLUE = '\033[94m'
	...
	while (1):
		...
		if (current_letter == "/") or (current_letter == "*"):
			...
			if (joined(current_tk) in COMMENT): # found a comment operator, now process the comment
				...
				# Found a comment! (Don't return it, skip it):
						...
						if (current_letter == "*" and next_letter == "/"):
							...
							if joined(current_tk) in COMMENT:
								try:
									current_tk.remove("/")
									current_tk.remove("*")
									# prepare current_letter and next_letter to be used from lex()
									current_letter = source_file.read(1)
									if current_letter == '\n':
										line += 1
									next_letter = peek_next_letter()
...
def add_variable_entity(name): 
	var_offset = scope_list[-1].update_offset(4) # make space for a new variable
...
def add_argument_entity(par_mode): 
	# Does not add anything, only generates and returns an argument					
	if par_mode == "in":
		mode = "CV"
	else:
		mode = "REF"
...
class quad():
	...
	def __str__(self): # For printing class elements
	...
	for q in quad_list:
		...
		if q.op == "begin_block":
			...
			if q.op == "=":
				...
				c_op = q.op
			# Write to file the appropriate expression:
...
def syn():
	...
	def doublewhile_stat(): 
		if token == "keyword-doublewhile_tk":
			state = newtemp()
			add_temp_variable_entity(state) 
			genquad(":=", "0", "", state)
			condquad = nextquad()
			get_next_token()
			if token == "open-paren_tk":
